...
overviewer.util = {
    ...
    'initialize': function() {
        ...
        overviewer.map.attributionControl.setPrefix('<a style="display: none;" href="https://overviewer.org">Overviewer/Leaflet</a>');
        ...
        overviewerConfig.tilesets.forEach(function(obj, idx) {
            var myLayer = new L.tileLayer('', {
                ...
                maxZoom: obj.maxZoom,
                minZoom: obj.minZoom,
                errorTileUrl: obj.base + "/icons/blank." + obj.imgextension,
            });
                ...
                if ((typeof markers !== 'undefined') && (obj.path in markers)) {
                    ...
                    for (var mkidx = 0; mkidx < markers[obj.path].length; mkidx++) {
                        ...
                        for (var dbidx = 0; dbidx < markersDB[marker_entry.groupName]?.raw.length; dbidx++) {
                            ...
                            } else */if (marker_entry.displayName == "Monsters") {
                              var icon =  L.divIcon({html: `<a style="color: #f3f577" class='marker'>${db.text}</a>`});
                            }/* else if (marker_entry.displayName == "Ores") {
                              var icon =  L.divIcon({html: `<a style="color: #f3f577">${db.text}</a>`});
                            } else if (marker_entry.displayName == "Runecrafting altars") {
                              var icon =  L.divIcon({html: `<a style="color: #f3f577">${db.text}</a>`});
                            } */ else {
                              var icon =  L.divIcon({html: `<img class="ov-marker" alt="${db.text}" src="${marker_entry.icon}">`});
                            }
    ...
    'getTileUrlGenerator': function(path, pathBase, pathExt) {
        return function(o) {
            ...
            return ("https://map.minescape.net/" + url);
        };
    },
